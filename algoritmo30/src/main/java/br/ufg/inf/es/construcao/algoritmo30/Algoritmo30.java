...
public class Algoritmo30 {
     ...
     * Deve conter um ou mais elementos, e seu ultimo elemento deve ser
     * diferente de zero.
    ...
    public static int polSimples(int x, int[] a) {
        if (a.length < 1) {
            throw new IllegalArgumentException("O tamanho do vetor deve ser "
                    + "maior ou igual a 1.");
        }
        if (a[a.length - 1] == 0) {
            throw new IllegalArgumentException("O ultimo elemento do vetor "
                    + "deve ser diferente de zero.");
        }

        int temp = Algoritmo5.potencia(x, a.length);
        int p = a[0];
        int i = 1;

        while (i <= a.length - 1) {
            temp = Algoritmo5.potencia(x, i);
            p += a[i] * temp;
            i++;
        }
