...
public class Algoritmo24 {
    ...
    /**
     * Calcula a divisao inteira de 'x' por 'y', somando quantas vezes 'x' pode
     * ser duplicado até chegar em 'y'.
     *
    ...
    public static int divideSomas(int x, int y) {
        if (x < 0) {
            throw new IllegalArgumentException("O 'x' deve ser "
                    + "maior ou igual a zero.");
        }
        if (y <= 0) {
            throw new IllegalArgumentException("O 'y' deve ser "
                    + "maior que zero.");
        }
